...
const Complaints = () => {
  ...
  const fetchComplaints = async (uid: string, role: string, permissions: RolePermission[] = []) => {
    try {
      ...
      // STRICT ISOLATION: Only admins can see all complaints, all other users see only their own data
      if (adminUser) {
  ...
  const handleView = (_complaint: Complaint) => {
    navigate(`/complaints/resolving`);
  };

  const handleEdit = (_complaint: Complaint) => {
