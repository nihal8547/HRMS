...
const StaffCreate = () => {
  ...
  const fetchLastEmployeeId = async () => {
    try {
      ...
      const staffsQuery = query(staffsRef, orderBy('employeeId', 'desc'), limit(1));
      const employeesQuery = query(employeesRef, orderBy('employeeId', 'desc'), limit(1));
      ...
      // Get last ID from staffs collection
      if (!staffsSnapshot.empty) {
        const staffId = staffsSnapshot.docs[0].data().employeeId;
        if (staffId && typeof staffId === 'string' && staffId.startsWith('EMP')) {
          lastId = staffId;
        }
      }
      
      // Get last ID from employees collection and compare
      if (!employeesSnapshot.empty) {
        const empId = employeesSnapshot.docs[0].data().employeeId;
        if (empId && typeof empId === 'string' && empId.startsWith('EMP')) {
          // Compare and get the higher one (lexicographically)
          if (!lastId || empId > lastId) {
            lastId = empId;
          }
        }
      }
      
      setLastEmployeeId(lastId || 'None');
    } catch (error) {
