...
const Settings = () => {
  ...
  const [activeTab, setActiveTab] = useState<'pages' | 'roles' | 'roleManagement' | 'departmentManagement' | 'userDeletion'>('pages');
  ...
  useEffect(() => {
    if (roles.length > 0 && !selectedRole) {
      // Always select Administrator role first if it exists, otherwise first role
      const adminRole = roles.find(r => r.name === 'Administrator');
      setSelectedRole(adminRole ? adminRole.name : roles[0].name);
    }
  ...
  const fetchPageControls = async () => {
    try {
      ...
      if (!snapshot.empty) {
        ...
        snapshot.docs.forEach(pageDoc => {
          ...
          if (!controlsMap.has(pageName)) {
            ...
            deleteDoc(doc(db, 'pageControls', pageDoc.id)).catch(err => 
      ...
      // Convert map to array, sorted by defaultPages order (consistent for all admins)
  ...
  const fetchRoles = async () => {
    try {
      ...
      if (!snapshot.empty) {
        ...
        // Sort roles consistently: Administrator first, then alphabetically
        rolesData.sort((a, b) => {
  ...
  const fetchRolePermissions = async () => {
    try {
      ...
      if (!snapshot.empty) {
        const permissions = snapshot.docs.map(permissionDoc => {
          const data = permissionDoc.data();
          const existingPages = data.pages || {};
          // Use the role field from data, or fallback to permissionDoc.id
          const roleName = data.role || permissionDoc.id;
        ...
        // Sort permissions consistently by role name (Administrator first, then alphabetically)
  ...
  const handleDeleteUser = async () => {
    ...
    if (!deleteEmployeeId && !deleteUserId) {
      setDeleteResult({
        success: false,
        message: 'Please enter either Employee ID or User ID.'
      });
      return;
    }

    const confirmMessage = deleteEmployeeId
      ? `Are you sure you want to permanently delete all data for employee ID: ${deleteEmployeeId}?\n\nThis will delete:\n- Database records (employees, staffs, userRoles)\n- Authentication account\n\nThis action CANNOT be undone!`
      : `Are you sure you want to permanently delete all data for user ID: ${deleteUserId}?\n\nThis will delete:\n- Database records (employees, staffs, userRoles)\n- Authentication account\n\nThis action CANNOT be undone!`;
    ...
    try {
      let result;
      if (deleteEmployeeId) {
        result = await deleteUserDataByEmployeeId(deleteEmployeeId, currentUser);
      } else {
        result = await deleteUserData(deleteUserId, currentUser);
      }
  ...
  return (
    <div className="settings-page">
      ...
      <div className="settings-tabs">
        ...
        <button
          className={`tab-button ${activeTab === 'userDeletion' ? 'active' : ''}`}
          onClick={() => setActiveTab('userDeletion')}
        >
          User Deletion
        </button>
      </div>
      ...
      {activeTab === 'roles' && (
        <div className="settings-container">
          ...
          <div className="role-permissions-grid">
            {[...pageControls]
              .sort((a, b) => a.pageName.localeCompare(b.pageName)) // Sort pages alphabetically for consistency across all admins
              .map((control) => {
      ...
      {activeTab === 'userDeletion' && (
        <div className="settings-container">
          <div className="user-deletion-section">
            <h3>User Data Deletion</h3>
            <div className="deletion-warning">
              <Icon name="alert-circle" />
              <p>
                <strong>Warning:</strong> This action will permanently delete user data from:
                <ul>
                  <li>Firestore Database (employees, staffs, userRoles collections)</li>
            ...
            <div className="deletion-form">
              <div className="form-group">
                <label>Delete by Employee ID</label>
                <input
                  type="text"
                ...
                />
                <small>Enter the employee ID to find and delete the user</small>
              </div>
              ...
              <div className="form-group">
                <label>Delete by User ID (UID)</label>
                <input
                  type="text"
                  value={deleteUserId}
                  onChange={(e) => {
                  ...
                  disabled={deletingUser}
                />
                <small>Enter the Firebase User ID (UID) to delete the user directly</small>
              </div>
              ...
              <div className="form-actions">
                <button
                  ...
                  disabled={deletingUser || (!deleteEmployeeId && !deleteUserId)}
                >
              ...
              {deleteResult && (
                <div className={`deletion-result ${deleteResult.success ? 'success' : 'error'}`}>
                  <Icon name={deleteResult.success ? 'check-circle' : 'alert-circle'} />
            ...
            <div className="deletion-info">
              ...
              <ul>
                <li>Employee record from <code>employees</code> collection</li>
                <li>Staff record from <code>staffs</code> collection</li>
                <li>User role from <code>userRoles</code> collection</li>
                <li>User account from Firebase Authentication</li>
                <li>All related documents linked by employeeId or authUserId</li>
              </ul>
              <p className="note">
                <strong>Note:</strong> To delete a user from Firebase Authentication, you may need to use Firebase Admin SDK 
                or Firebase Console if the user is not the currently logged-in user. The client SDK can only delete the current user's own account.
              </p>
            </div>
          </div>
        </div>
